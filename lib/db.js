...
// Keep one connection alive across hot-reloads in dev.
let _db = globalThis.__agrimall_db;
...
function migrate(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS orders (
      id            TEXT PRIMARY KEY,          -- e.g. AGM-4821
      customer_name TEXT,
      phone         TEXT,
      address       TEXT,
      pincode       TEXT,
      ...
      total         INTEGER,
      payment       TEXT,
      status        TEXT DEFAULT 'placed',     -- lifecycle stage key
      seller_id     INTEGER,
      eta           TEXT,                       -- ISO date, expected arrival
      created_at    TEXT DEFAULT (datetime('now')),
    ...
    CREATE TABLE IF NOT EXISTS tracking_events (
      id            INTEGER PRIMARY KEY AUTOINCREMENT,
      order_id      TEXT NOT NULL,
      stage         TEXT NOT NULL,             -- lifecycle stage key
      title         TEXT,
      note          TEXT,
      location      TEXT,
      pct           INTEGER,
...
function seedIfEmpty(db) {
  ...
  const seed = db.transaction(() => {
    // Demo + registered nursery sellers
  ...
  // Seed a handful of historical orders (already progressed) so dashboards aren't empty.
...
function seedHistoricalOrders(db) {
  ...
  const insOrder = db.prepare(`
    INSERT INTO orders (id,customer_name,region,city,subtotal,shipping,total,payment,status,eta,created_at,updated_at)
  ...
  const tx = db.transaction(() => {
    for (const o of ALL_ORDERS) {
      ...
      // Backfill tracking events up to the current stage.
